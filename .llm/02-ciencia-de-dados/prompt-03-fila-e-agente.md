...
1. A TABELA DA SEMANA: gold.fila_semanal
   ...
   Subquery escalar CORRELACIONADA precisa ser agregada para devolver uma
   linha: envolva o valor em max(...) (ex.: max(concat(sug.sku, ' - saldo ',
   saldo))). Sem isso o engine recusa com "Correlated scalar subqueries must
   be aggregated to return at most one row".
