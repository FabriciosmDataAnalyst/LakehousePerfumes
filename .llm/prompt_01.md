...
databricks catalogs list --profile perfumariaaula | grep rotaperfume || echo 'não existe'
...
```bash
databricks fs ls dbfs:/Volumes/lakehouse_rotaperfume/bronze/raw/erp --profile perfumariaaula
```
...
databricks fs rm dbfs:/Volumes/lakehouse_rotaperfume/bronze/raw/erp/pagamentos.csv \
  --profile perfumariaaula
databricks bundle run rotaperfume_pipeline --target dev --profile perfumariaaula
# a tarefa raw_conferencia FALHA e o job para: falta pagamentos.csv
bash scripts/subir-raw.sh perfumariaaula     # devolve o arquivo e rode de novo
```
...
| `databricks experimental aitools tools query` divide o SQL nos espaços | A query veio como argumento posicional | Passe por stdin: `echo "SQL" \| databricks experimental aitools tools query --output json` |
| `Syntax error at or near 'USING'` no `CREATE TABLE` | `USING DELTA` depois do `COMMENT` | Escreva `USING DELTA` **antes** do `COMMENT` da tabela |
| `databricks` não encontrado em scripts `.sh` no Windows | `bash` não herda o PATH do PowerShell | Rode o CLI diretamente ou chame o binário explícito (`databricks.exe`) |
