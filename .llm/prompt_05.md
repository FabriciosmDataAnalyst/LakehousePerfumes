...
| `BAD_REQUEST: Circular reference detected in calculated field: receita` | Métrica com o mesmo nome de uma coluna crua do SELECT (ex.: medida `Receita` e coluna `receita`) | Aliasie no SELECT: `receita AS receita_linha`, e use o alias no `expression` da medida. Nome de métrica nunca colide com coluna |
